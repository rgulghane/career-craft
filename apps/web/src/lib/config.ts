...
export const serverConfig = {
  ...
  azureStorage: {
    /** Full connection string (simplest auth — overrides managed identity). */
    get connectionString() {
      return readString("AZURE_STORAGE_CONNECTION_STRING", "");
    },
    /** Storage account name (used with managed identity when no connection string). */
    get accountName() {
      return readString("AZURE_STORAGE_ACCOUNT_NAME", "");
    },
    /** Blob container that holds mentor photos. */
    get containerName() {
      return readString("AZURE_STORAGE_CONTAINER_NAME", "mentor-photos");
    },
    /**
     * Optional public base URL (e.g. an Azure CDN / Front Door custom domain).
     * When set, returned image URLs use this host instead of the raw blob host.
     * No trailing slash.
     */
    get publicBaseUrl() {
      return readString("AZURE_STORAGE_PUBLIC_BASE_URL", "").replace(/\/+$/, "");
    },
    get configured() {
      return Boolean(
        process.env.AZURE_STORAGE_CONNECTION_STRING || process.env.AZURE_STORAGE_ACCOUNT_NAME,
      );
    },
  },
  razorpay: {
